  ...
  return parsedEntries.map((entry) => {
    ...
    if (Array.isArray(entry.recurring)) {
      normalizedEntry.recurring = entry.recurring.map((daySchedule) => ({
        day: normalizeDayName(daySchedule.day),
    ...
    // CRITICAL: Also normalize time_slots for one-time availability
    if (Array.isArray(entry.time_slots)) {
      normalizedEntry.time_slots = entry.time_slots.map((slot) => ({
        start: normalizeTime(slot.start),
        end: normalizeTime(slot.end),
      }));
    } else {
      normalizedEntry.time_slots = [];
    }

    console.log("Normalized entry:", JSON.stringify(normalizedEntry, null, 2));
...
function detectConflicts(newEntries, existingEntries) {
  ...
  newEntries.forEach((newEntry) => {
    ...
    console.log("Checking new entry:", {
      start: newStart.toISOString(),
      end: newEnd.toISOString(),
      hasRecurring:
        Array.isArray(newEntry.recurring) && newEntry.recurring.length > 0,
      hasTimeSlots:
        Array.isArray(newEntry.time_slots) && newEntry.time_slots.length > 0,
      recurringDays: newEntry.recurring?.map((r) => r.day) || [],
      timeSlots: newEntry.time_slots || [],
    });
    ...
    existingEntries.forEach((existing) => {
      ...
      if (datesOverlap(newStart, newEnd, existStart, existEnd)) {
        ...
        // Check for one-time time slot conflicts (NEW LOGIC)
        if (
          Array.isArray(newEntry.time_slots) &&
          Array.isArray(existing.time_slots)
        ) {
          // For one-time availability, if dates overlap, check time slots
          newEntry.time_slots.forEach((newSlot) => {
            existing.time_slots.forEach((existSlot) => {
              if (timeOverlaps(newSlot, existSlot)) {
                conflicts.push({
                  type: "time_conflict",
                  message: `Time conflict on specific date`,
                  details: `Your requested time ${newSlot.start}-${
                    newSlot.end
                  } overlaps with existing ${existSlot.start}-${
                    existSlot.end
        ...
        // Check mixed conflicts (recurring vs one-time)
        if (
          Array.isArray(newEntry.recurring) &&
          Array.isArray(existing.time_slots)
        ) {
          newEntry.recurring.forEach((newRec) => {
            // Check if this recurring day falls within the overlapping date range
            const dayOverlaps = checkDayInDateRange(
              newRec.day,
              overlapStart,
              overlapEnd
                ...
                existing.time_slots.forEach((existSlot) => {
                  if (timeOverlaps(newSlot, existSlot)) {
                    conflicts.push({
                      type: "time_conflict",
                      message: `Time conflict on ${newRec.day}`,
                      details: `Your requested recurring time ${
                        newSlot.start
                      }-${newSlot.end} on ${
                        newRec.day
                      }s overlaps with existing one-time schedule ${
                        existSlot.start
                      }-${
                        existSlot.end
                      } between ${overlapStart.toLocaleDateString()} and ${overlapEnd.toLocaleDateString()}`,
        ...
        // Check mixed conflicts (one-time vs recurring)
        if (
          Array.isArray(newEntry.time_slots) &&
          Array.isArray(existing.recurring)
        ) {
              ...
              newEntry.time_slots.forEach((newSlot) => {
                existRec.time_slots.forEach((existSlot) => {
                  if (timeOverlaps(newSlot, existSlot)) {
                    conflicts.push({
                      type: "time_conflict",
                      message: `Time conflict on ${existRec.day}`,
                      details: `Your requested one-time time ${newSlot.start}-${
                        newSlot.end
                      } overlaps with existing recurring schedule ${
                        existSlot.start
                      }-${existSlot.end} on ${
                        existRec.day
                      }s between ${overlapStart.toLocaleDateString()} and ${overlapEnd.toLocaleDateString()}`,
                      day: existRec.day,
                      requestedTime: `${newSlot.start}-${newSlot.end}`,
                      existingTime: `${existSlot.start}-${existSlot.end}`,
                      conflictDateRange: `${overlapStart.toLocaleDateString()} - ${overlapEnd.toLocaleDateString()}`,
...
function checkDayInDateRange(day, startDate, endDate) {
  ...
  if (dayIndex === -1) {
    console.log(`Invalid day name: ${day}`);
    return false;
  }
